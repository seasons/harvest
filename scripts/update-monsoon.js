...
async function main() {
  try {
    await updateRepo({
      repo: { owner: "seasons", repo: "monsoon" },
      branch: "update-harvest-query-map",
      message: "Update harvest query map",
      update: (dir) => {
        exec("node_modules/.bin/persistgraphql src data/complete.queryMap.json --js --extension=tsx", process.cwd())

        log.step("Merging complete.queryMap.json")
        // merge metaphysics into emission map first, to preserve any manual edits made in MP
        mergeJson("data/complete.queryMap.json", path.join(dir, "src/tests/complete.queryMap.json"))
        // then merge back into metaphysics to update
        mergeJson(path.join(dir, "src/tests/complete.queryMap.json"), "data/complete.queryMap.json")
      },
    })
  } catch (e) {
...
async function createAndMergePullRequest({ repo, branch, title }) {
  ...
  const res = await octokit.pulls.create({
    ...
    base: "master",
    title: title,
    body: "Greetings human :robot: This PR was created as part of harvest's automated deployment.",
  })
